...
import "./App.css";
import KycNFT from './contracts/KycNFT.json'; // Importing the NFT contract
const crypto = require("crypto");

const generateMetadataURI = (kycData) => {
  const data = {};
  kycData.forEach((value, key) => {
    data[key] = value;
  });

  const metadata = {
    name: "KYC NFT",
    description: "KYC NFT for user",
    attributes: data,
  };
...
class App extends Component {
  state = {
    ...
    panVerify: null,
    verified: null,
    nftContract: null, // State for NFT contract
    tokenId: null, // State to store the token ID of the minted NFT
  };
  ...
  componentDidMount = async () => {
    try {
      ...
      const nftNetwork = KycNFT.networks[networkId];
      const nftInstance = new web3.eth.Contract(
        KycNFT.abi,
        nftNetwork && nftNetwork.address,
      );
      this.setState({ nftContract: nftInstance });
    } catch (error) {
  ...
  createmycustomer = async (event) => {
    event.preventDefault();
    var { contract, nftContract, account } = this.state;
    await contract.methods
      .newCustomer(
        this.state.name,
        crypto.createHash("sha1").update(this.state.aadhar + this.state.pan).digest("hex"),
        this.state.bank_verify
      )
      .send({ from: account })
      .then(() => {
        window.alert("You successfully made an account and NFT is generated!");
      });

    // Mint an NFT for the new customer
    const kycData = new Map([
      ['name', this.state.name],
      ['aadhar', this.state.aadhar],
      ['pan', this.state.pan]
    ]);
    const uri = generateMetadataURI(kycData);
    console.log('Minting NFT...');
    const receipt = await nftContract.methods.mintKycNFT(account, uri).send({ from: account });
    const tokenId = receipt.events.Transfer.returnValues.tokenId;
    this.setState({ tokenId });
    console.log(`NFT Minted! Token ID: ${tokenId}`);
    window.alert(`NFT Minted! Token ID: ${tokenId}`);
  };
  ...
  verifykycfromcustomer = async () => {
    var { nftContract, tokenId } = this.state;
    const uri = await nftContract.methods.tokenURI(tokenId).call();
    const kycData = JSON.parse(atob(uri.split(',')[1]));
    this.setState({
      name: kycData.attributes.name,
      aadhar: kycData.attributes.aadhar,
      pan: kycData.attributes.pan,
      verified: "Success"
    });
  };
  ...
  render() {
    ...
    return (
      <div className="App">
        ...
        <div className="navbar navbar-expand-lg navbar-dark bg-dark fixed-top justify-content-center">
          <button onClick={() => show("new-customer")} id="new-customer-button">
            New customer
          </button>
          <button onClick={() => show("existing-customer")} id="existing-customer-button">
            Existing customer
          </button>
          <button onClick={() => show("new-bank")} id="new-bank-button">
            New Organisation
          </button>
          <button onClick={() => show("existing-bank")} id="existing-bank-button">
        ...
        <div className="new-customer form-top-padding">
          <form action="." method="" onSubmit={this.create_customer}>
            <fieldset>
              ...
              <p>
                <label>Organisation Address you want to verify your data with </label>
                <input type="text" onChange={this.myVBankChangeHandler} />
              </p>
              <p>
                <button type="submit" onClick={this.createmycustomer}>Create Customer and Mint NFT</button>
              </p>
              {this.state.tokenId && (
                <div>
                  <p>Generated Token ID: {this.state.tokenId}</p>
                </div>
              )}
            </fieldset>
          </form>
        ...
        <div className="existing-bank">
          ...
          <div>
            ...
            <p>
              ...
              <input type="text" onChange={this.myTokenIdChangeHandler} />
            </p>
